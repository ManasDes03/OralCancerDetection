...
# Load the CSV file
data = pd.read_csv('../data/Sri Lankan Dataset/Patientwise_Data.csv')  # Replace with actual file path

# Create a PDF file to save all plots
pdf_filename = "statistics_sri_lankan_patientwise.pdf"

with PdfPages(pdf_filename) as pdf:
    # Categorical columns for pie charts
    categorical_columns = ['Gender', 'Smoking', 'Chewing_Betel_Quid ', 'Alcohol']
    for column in categorical_columns:
        plt.figure(figsize=(6, 6))
        value_counts = data[column].value_counts()
        labels = [f'{index} ({value})' for index, value in zip(value_counts.index, value_counts.values)]
        value_counts.plot.pie(autopct=lambda p: f'{p:.1f}%\n({int(p*sum(value_counts)/100)})', startangle=90, cmap='Pastel1', labels=labels)
    ...
    numerical_columns = ['Age', 'Image Count']

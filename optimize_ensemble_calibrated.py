"""
Calibrated Ensemble Optimizer (Recall-Constrained)
=================================================

Purpose:
- Improve clinical operating point without retraining base CNNs.
- Calibrate model probabilities on validation split.
- Optimize threshold under recall constraint.
- Report statistical significance vs positive baseline.

Outputs:
- gdc_suspicious_results_ensemble/calibrated_optimization_report.json
- gdc_suspicious_results_ensemble/calibrated_optimization_summary.txt
"""
...
def main():
    print("\nCalibrated Ensemble Optimization")
    print("=" * 70)
    ...
    print("\n4) Probability calibration on validation split...")
    # Platt scaling for each model output
    platt_pos = LogisticRegression(random_state=SEED, max_iter=1000, class_weight="balanced")
    platt_pos.fit(ppos_val.reshape(-1, 1), y_val)
    ppos_val_platt = platt_pos.predict_proba(ppos_val.reshape(-1, 1))[:, 1]
    ppos_test_platt = platt_pos.predict_proba(ppos_test.reshape(-1, 1))[:, 1]

    platt_neg = LogisticRegression(random_state=SEED, max_iter=1000, class_weight="balanced")
    platt_neg.fit(pneg_susp_val.reshape(-1, 1), y_val)
    pneg_val_platt = platt_neg.predict_proba(pneg_susp_val.reshape(-1, 1))[:, 1]
    pneg_test_platt = platt_neg.predict_proba(pneg_susp_test.reshape(-1, 1))[:, 1]

    # Isotonic calibration
    iso_pos = IsotonicRegression(out_of_bounds="clip")
    iso_pos.fit(ppos_val, y_val)
    ...
    iso_neg.fit(pneg_susp_val, y_val)
    pneg_val_iso = iso_neg.transform(pneg_susp_val)
    pneg_test_iso = iso_neg.transform(pneg_susp_test)

    print("\n5) Build candidate ensemble scores...")
    candidates_val: Dict[str, np.ndarray] = {}
    candidates_test: Dict[str, np.ndarray] = {}

    # Raw candidates
    candidates_val["raw_positive"] = ppos_val
    candidates_test["raw_positive"] = ppos_test

    candidates_val["avg_raw"] = 0.5 * (ppos_val + pneg_susp_val)
    candidates_test["avg_raw"] = 0.5 * (ppos_test + pneg_susp_test)

    # Calibrated averages
    candidates_val["avg_platt"] = 0.5 * (ppos_val_platt + pneg_val_platt)
    candidates_test["avg_platt"] = 0.5 * (ppos_test_platt + pneg_test_platt)

    candidates_val["avg_isotonic"] = 0.5 * (ppos_val_iso + pneg_val_iso)
    candidates_test["avg_isotonic"] = 0.5 * (ppos_test_iso + pneg_test_iso)

    # Contradiction-penalized calibrated score
    dis_val = np.abs(ppos_val_platt - pneg_val_platt)
    dis_test = np.abs(ppos_test_platt - pneg_test_platt)
    candidates_val["contra_platt"] = (0.5 * (ppos_val_platt + pneg_val_platt)) * (1.0 - 0.5 * dis_val)
    candidates_test["contra_platt"] = (0.5 * (ppos_test_platt + pneg_test_platt)) * (1.0 - 0.5 * dis_test)

    # Stacked calibrated meta-learner
    X_val = np.column_stack([
        ppos_val_platt,
        pneg_val_platt,
        np.abs(ppos_val_platt - pneg_val_platt),
        ppos_val_platt * pneg_val_platt,
        ppos_val_iso,
        pneg_val_iso,
    ])
    X_test = np.column_stack([
        ppos_test_platt,
        pneg_test_platt,
        np.abs(ppos_test_platt - pneg_test_platt),
        ppos_test_platt * pneg_test_platt,
        ppos_test_iso,
        pneg_test_iso,
    ])
    ...
    candidates_val["stacked_calibrated"] = stacker.predict_proba(X_val)[:, 1]
    candidates_test["stacked_calibrated"] = stacker.predict_proba(X_test)[:, 1]

    print("\n6) Tune threshold under recall constraint on validation...")
    tuned = {}
    for name, s_val in candidates_val.items():
        best = find_threshold_recall_constrained(y_val, s_val, min_recall=target_recall)
        if best is not None:
            tuned[name] = best

    if not tuned:
        raise RuntimeError("No candidate met recall constraint on validation.")

    winner_name = sorted(
        tuned.keys(),
        key=lambda n: (tuned[n]["specificity"], tuned[n]["accuracy"]),
        reverse=True,
    )[0]
    winner_thr = tuned[winner_name]["threshold"]

    print(f"   Winner: {winner_name}")
    print(f"   Threshold: {winner_thr:.3f}")

    print("\n7) Test-set evaluation (untouched)...")
    baseline = evaluate(y_test, candidates_test["raw_positive"], tuned["raw_positive"]["threshold"], "positive_baseline")
    optimized = evaluate(y_test, candidates_test[winner_name], winner_thr, winner_name)
    ...
    pred_base = (candidates_test["raw_positive"] > tuned["raw_positive"]["threshold"]).astype(int)
    pred_opt = (candidates_test[winner_name] > winner_thr).astype(int)

    p_mcnemar, b, c = mcnemar_pvalue(y_test, pred_base, pred_opt)
    rec_ci, fp_ci = bootstrap_diff_ci(
        y_test,
        candidates_test["raw_positive"],
        tuned["raw_positive"]["threshold"],
        candidates_test[winner_name],
        winner_thr,
        n_boot=2000,
    )
    ...
    report = {
        ...
        "validation_tuned_candidates": tuned,
        "winner": winner_name,
        "winner_threshold": float(winner_thr),
        "test_metrics": {"baseline": as_dict(baseline), "optimized": as_dict(optimized)},
        "improvement": {
        ...
        "significance": {
            ...
            "mcnemar_b": int(b),
            "mcnemar_c": int(c),
            "recall_diff_95ci": [float(rec_ci[0]), float(rec_ci[1])],
            "fp_diff_95ci": [float(fp_ci[0]), float(fp_ci[1])],
        },
    }

    json_path = os.path.join(out_dir, "calibrated_optimization_report.json")
    txt_path = os.path.join(out_dir, "calibrated_optimization_summary.txt")

    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(report, f, indent=2)

    with open(txt_path, "w", encoding="utf-8") as f:
        f.write("Calibrated Ensemble Optimization Summary\n")
        f.write("=" * 44 + "\n\n")
        f.write(f"Winner: {winner_name}\n")
        f.write(f"Threshold: {winner_thr:.3f}\n")
        f.write(f"Recall target: {target_recall:.2f}\n\n")
        f.write("Baseline:\n")
        ...
        f.write(f"  Recall diff 95% CI={rec_ci}\n")
